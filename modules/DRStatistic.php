   ...
   */
    public function initStats($players)
    {
        ...
        foreach ($players as $player_id => $player) {
            ...
            $this->game->initStat('player', STAT_XP_LEVEL, 0, $player_id);

            $this->game->initStat('player', STAT_XP_TREASURE, 0, $player_id);
            $this->game->initStat('player', STAT_XP_DRAGON, 0, $player_id);
            $this->game->initStat('player', STAT_XP_DRAGON_SCALE, 0, $player_id);
            $this->game->initStat('player', STAT_XP_TOWN_PORTAL, 0, $player_id);

            $this->game->initStat('player', STAT_NBR_DRAGON_KILL, 0, $player_id);
            $this->game->initStat('player', STAT_NBR_TREASURE_OPEN, 0, $player_id);
        }
    }
    ...
    public function incDragonKilled($nbr_dragons) {
        $player_id = $this->game->getActivePlayerId();
        $this->game->incStat(1, STAT_XP_DRAGON, $player_id);
        $this->game->incStat($nbr_dragons, STAT_NBR_DRAGON_KILL, $player_id);
    }

    public function incTreasureOpen($nbr_treasures) {
        $player_id = $this->game->getActivePlayerId();
        $this->game->incStat($nbr_treasures, STAT_NBR_TREASURE_OPEN, $player_id);
    }
}
